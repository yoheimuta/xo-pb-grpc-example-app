...
func (ua *UserAuth) Update(ctx context.Context, db XODB) error {
	var err error
	...
	_, err = db.ExecContext(ctx, sqlstr, ua.Email, ua.PasswordHash, ua.CreatedAt, ua.UpdatedAt, ua.UserID)
	return err
}

// Save saves the UserAuth to the database.
func (ua *UserAuth) Save(ctx context.Context, db XODB) error {
	if ua.Exists() {
		return ua.Update(ctx, db)
	}

	return ua.Insert(ctx, db)
}
...
func UserAuthByEmail(ctx context.Context, db XODB, email string) (*UserAuth, error) {
	var err error
	...
	err = db.QueryRowContext(ctx, sqlstr, email).Scan(&ua.UserID, &ua.Email, &ua.PasswordHash, &ua.CreatedAt, &ua.UpdatedAt)
	if err != nil {
		return nil, err
	}
...
func UserAuthByUserID(ctx context.Context, db XODB, userID string) (*UserAuth, error) {
	var err error
	...
	err = db.QueryRowContext(ctx, sqlstr, userID).Scan(&ua.UserID, &ua.Email, &ua.PasswordHash, &ua.CreatedAt, &ua.UpdatedAt)
	if err != nil {
		return nil, err
	}
