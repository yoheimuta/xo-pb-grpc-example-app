...
type UserProduct struct {
	UserProductID int    `json:"user_product_id"` // user_product_id
...
func (up *UserProduct) Insert(db XODB) error {
	...
	const sqlstr = `INSERT INTO test-xo-db.user_products (` +
		`user_product_id, user_id, title, description` +
		`) VALUES (` +
		`?, ?, ?, ?` +
		`)`

	// run query
	XOLog(sqlstr, up.UserProductID, up.UserID, up.Title, up.Description)
	_, err = db.Exec(sqlstr, up.UserProductID, up.UserID, up.Title, up.Description)
	if err != nil {
		return err
	}
...
func (up *UserProduct) Update(db XODB) error {
	...
	// run query
	XOLog(sqlstr, up.UserID, up.Title, up.Description, up.UserProductID)
	_, err = db.Exec(sqlstr, up.UserID, up.Title, up.Description, up.UserProductID)
	return err
}
...
func UserProductsByUserID(db XODB, userID int) ([]*UserProduct, error) {
	var err error
	...
	for q.Next() {
		...
		// scan
		err = q.Scan(&up.UserProductID, &up.UserID, &up.Title, &up.Description)
		if err != nil {
			return nil, err
		}
...
func UserProductByUserProductID(db XODB, userProductID int) (*UserProduct, error) {
	var err error
	...
	err = db.QueryRow(sqlstr, userProductID).Scan(&up.UserProductID, &up.UserID, &up.Title, &up.Description)
	if err != nil {
		return nil, err
	}
